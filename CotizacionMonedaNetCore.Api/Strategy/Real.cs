...
{
    public class Real : ICambioMoneda
    {
        /// <summary>
        ...
        /// <returns> retorna la cotizacion de la moneda comparada al dolar estadounidense</returns>
        public MonedaModels Cotizar(string url, string key)
        {
            var result = HttpClientRequets.Request<ConversionMonedaModels>(
                $"{url}BRL/ARS/json?quantity=1&key={key}",
                HttpMethodEnum.Get);
